...
const RING_SIZE: usize = 16*1024*1024;
...
fn worker_fn(args: &mut WorkerArg) -> ! {
    ...
    loop {
        ...
        output.clear();
        libos.wait_for_something(&qtokens, &mut output);
...
pub fn main() -> Result<()> {
    ...
    if args.len() >= 5 {
        if args[1] == "--server" {
            ...
            let ring_name = format!("Main_Ring\0");
            let q_ptr: *mut DPDKRing = Box::into_raw(Box::new(DPDKRing::new(ring_name, RING_SIZE as u32)));
