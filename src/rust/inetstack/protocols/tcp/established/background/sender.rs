...
pub async fn sender<N: NetworkRuntime>(cb: *mut SharedControlBlock<N>) -> Result<Never, Fail> {
    'top: loop {
        ...
        let mut unsent_seq_watched: SharedAsyncValue<SeqNumber> = unsafe { (*cb).get_unsent_seq_no() };
        let unsent_seq: SeqNumber = unsent_seq_watched.get();

        let mut send_next_watched: SharedAsyncValue<SeqNumber> = unsafe { (*cb).get_send_next() };
        ...
        let mut win_sz_watched: SharedAsyncValue<u32> = unsafe { (*cb).get_send_window() };
        ...
        if win_sz == 0 {
            ...
            let remote_link_addr = unsafe { (*cb).arp().query((*cb).get_remote().ip().clone()).await? };
            let buf: DemiBuffer = unsafe { (*cb)
                .pop_one_unsent_byte()
                .unwrap_or_else(|| panic!("No unsent data? {}, {}", send_next, unsent_seq)) };

            // Update SND.NXT.
            unsafe { (*cb).modify_send_next(|s| s + SeqNumber::from(1)) };

            // Add the probe byte (as a new separate buffer) to our unacknowledged queue.
            let unacked_segment = UnackedSegment {
                bytes: buf.clone(),
                initial_tx: Some(unsafe { (*cb).get_now() }),
            };
            unsafe { (*cb).push_unacked_segment(unacked_segment) };
            ...
            loop {
                ...
                unsafe { (*cb).emit(header, Some(buf.clone()), remote_link_addr) };
        ...
        let mut send_unacked_watched: SharedAsyncValue<SeqNumber> = unsafe { (*cb).get_send_unacked() };
        ...
        let mut cwnd_watched: SharedAsyncValue<u32> = unsafe { (*cb).congestion_control_get_cwnd() };
        let cwnd: u32 = cwnd_watched.get();

        // The limited transmit algorithm may increase the effective size of cwnd by up to 2 * mss.
        let mut ltci_watched: SharedAsyncValue<u32> = unsafe { (*cb).congestion_control_get_limited_transmit_cwnd_increase() };
        let ltci: u32 = ltci_watched.get();

        let effective_cwnd: u32 = cwnd + ltci;
        let next_buf_size: usize = expect_some!(unsafe { (*cb).unsent_top_size() }, "no buffer in unsent queue");

        let sent_data: u32 = (send_next - send_unacked).into();
        if win_sz <= (sent_data + next_buf_size as u32)
            || effective_cwnd <= sent_data
            || (effective_cwnd - sent_data) <= unsafe { (*cb).get_mss() } as u32
        {
        ...
        let max_size: usize = cmp::min(
            cmp::min((win_sz - sent_data) as usize, unsafe { (*cb).get_mss() }),
            (effective_cwnd - sent_data) as usize,
        );
        let (segment_data, do_push): (DemiBuffer, bool) = expect_some!(
            unsafe { (*cb).pop_unsent_segment(max_size) },
        ...
        unsafe { (*cb).emit(header, Some(segment_data.clone()), remote_link_addr) };

        // Update SND.NXT.
        unsafe { (*cb).modify_send_next(|s| s + SeqNumber::from(segment_data_len)) };
        ...
        let retransmit_deadline = unsafe { (*cb).get_retransmit_deadline() };
        if retransmit_deadline.is_none() {
            let rto: Duration = unsafe { (*cb).rto() };
            unsafe { (*cb).set_retransmit_deadline(Some((*cb).get_now() + rto)) };
        }
    }
}
