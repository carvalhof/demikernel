...
impl Scheduler {
    ...
    fn poll(&mut self, group_index: usize, ready_indices: &mut Vec<usize>) -> usize {
        debug_assert!(group_index < self.groups.len());

        let mut polled_tasks: usize = 0;

        ready_indices.clear();
        // let mut ready_indices: Vec<usize> = Vec::<usize>::with_capacity(1000);
        self.groups[group_index].get_offsets_for_ready_tasks(ready_indices);
    ...
    pub fn poll_all(&mut self, ready_indices: &mut Vec<usize>) -> usize {
        let mut polled_tasks: usize = 0;
        for i in 0..self.groups.len() {
            polled_tasks += self.poll(i, ready_indices);
        }
        polled_tasks
    }
    ...
    pub fn poll_group(&mut self, group_id: TaskId) -> Option<usize> {
        let mut ready_indices: Vec<usize> = Vec::<usize>::with_capacity(1000);
        Some(self.poll(self.ids.get(&group_id)?.into(), &mut ready_indices))
    }
