...
impl SingleThreadLibOS {
    ...
    pub fn new(libos_name: LibOSName) -> Result<Self, Fail> {
        ...
        let libos: SingleThreadLibOS = match libos_name {
            #[cfg(all(feature = "catnap-libos"))]
    ...
    pub fn create_pipe(&mut self, name: &str) -> Result<QDesc, Fail> {
        let result: Result<QDesc, Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(_) => Err(Fail::new(
                    libc::ENOTSUP,
                    "create_pipe() is not supported on network liboses",
                )),
                SingleThreadLibOS::MemoryLibOS(libos) => libos.create_pipe(name),
            }
        };
    ...
    pub fn open_pipe(&mut self, name: &str) -> Result<QDesc, Fail> {
        let result: Result<QDesc, Fail> = {
            ...
            match self {
                ...
                SingleThreadLibOS::MemoryLibOS(libos) => libos.open_pipe(name),
            }
        };
    ...
    pub fn socket(
        ...
        let result: Result<QDesc, Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => libos.socket(domain, socket_type, protocol),
                SingleThreadLibOS::MemoryLibOS(_) => {
                    Err(Fail::new(libc::ENOTSUP, "socket() is not supported on memory liboses"))
                },
            }
        };
    ...
    pub fn bind(&mut self, sockqd: QDesc, local: SocketAddr) -> Result<(), Fail> {
        let result: Result<(), Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => libos.bind(sockqd, local),
                SingleThreadLibOS::MemoryLibOS(_) => {
                    Err(Fail::new(libc::ENOTSUP, "bind() is not supported on memory liboses"))
                },
            }
        };
    ...
    pub fn listen(&mut self, sockqd: QDesc, backlog: usize) -> Result<(), Fail> {
        let result: Result<(), Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => libos.listen(sockqd, backlog),
                SingleThreadLibOS::MemoryLibOS(_) => {
                    Err(Fail::new(libc::ENOTSUP, "listen() is not supported on memory liboses"))
                },
            }
        };
    ...
    pub fn accept(&mut self, sockqd: QDesc) -> Result<QToken, Fail> {
        let result: Result<QToken, Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => libos.accept(sockqd),
                SingleThreadLibOS::MemoryLibOS(_) => {
                    Err(Fail::new(libc::ENOTSUP, "accept() is not supported on memory liboses"))
                },
            }
        };
    ...
    pub fn connect(&mut self, sockqd: QDesc, remote: SocketAddr) -> Result<QToken, Fail> {
        let result: Result<QToken, Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => libos.connect(sockqd, remote),
                SingleThreadLibOS::MemoryLibOS(_) => {
                    Err(Fail::new(libc::ENOTSUP, "connect() is not supported on memory liboses"))
                },
            }
        };
    ...
    pub fn close(&mut self, qd: QDesc) -> Result<(), Fail> {
        let result: Result<(), Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => match libos.async_close(qd) {
                ...
                SingleThreadLibOS::MemoryLibOS(libos) => match libos.async_close(qd) {
                    Ok(qt) => match self.wait(qt, None) {
                        Ok(_) => Ok(()),
                        Err(e) => Err(e),
                    },
    ...
    pub fn async_close(&mut self, qd: QDesc) -> Result<QToken, Fail> {
        let result: Result<QToken, Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => libos.async_close(qd),
                SingleThreadLibOS::MemoryLibOS(libos) => libos.async_close(qd),
            }
        };
    ...
    pub fn push(&mut self, qd: QDesc, sga: &demi_sgarray_t) -> Result<QToken, Fail> {
        let result: Result<QToken, Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => libos.push(qd, sga),
                SingleThreadLibOS::MemoryLibOS(libos) => libos.push(qd, sga),
            }
        };
    ...
    pub fn pushto(&mut self, qd: QDesc, sga: &demi_sgarray_t, to: SocketAddr) -> Result<QToken, Fail> {
        let result: Result<QToken, Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => libos.pushto(qd, sga, to),
                SingleThreadLibOS::MemoryLibOS(_) => {
                    Err(Fail::new(libc::ENOTSUP, "pushto() is not supported on memory liboses"))
                },
            }
        };
    ...
    pub fn pop(&mut self, qd: QDesc, size: Option<usize>) -> Result<QToken, Fail> {
        let result: Result<QToken, Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => libos.pop(qd, size),
                SingleThreadLibOS::MemoryLibOS(libos) => libos.pop(qd, size),
            }
        };
    ...
    pub fn wait(&mut self, qt: QToken, timeout: Option<Duration>) -> Result<demi_qresult_t, Fail> {
        ...
        match self {
            SingleThreadLibOS::NetworkLibOS(libos) => libos.wait(qt, timeout.unwrap_or(DEFAULT_TIMEOUT)),
            SingleThreadLibOS::MemoryLibOS(libos) => libos.wait(qt, timeout.unwrap_or(DEFAULT_TIMEOUT)),
        }
    }
    ...
    pub fn wait_any(&mut self, qts: &[QToken], timeout: Option<Duration>) -> Result<(usize, demi_qresult_t), Fail> {
        ...
        match self {
            SingleThreadLibOS::NetworkLibOS(libos) => libos.wait_any(qts, timeout.unwrap_or(DEFAULT_TIMEOUT)),
            SingleThreadLibOS::MemoryLibOS(libos) => libos.wait_any(qts, timeout.unwrap_or(DEFAULT_TIMEOUT)),
        }
    }
    ...
    pub fn sgaalloc(&mut self, size: usize) -> Result<demi_sgarray_t, Fail> {
        let result: Result<demi_sgarray_t, Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => libos.sgaalloc(size),
                SingleThreadLibOS::MemoryLibOS(libos) => libos.sgaalloc(size),
            }
        };
    ...
    pub fn sgafree(&mut self, sga: demi_sgarray_t) -> Result<(), Fail> {
        let result: Result<(), Fail> = {
            ...
            match self {
                SingleThreadLibOS::NetworkLibOS(libos) => libos.sgafree(sga),
                SingleThreadLibOS::MemoryLibOS(libos) => libos.sgafree(sga),
            }
        };
    ...
    /// Polls I/O completions.
    fn poll(&mut self) {
        #[cfg(feature = "profiler")]
        timer!("demikernel::poll");
        match self {
            SingleThreadLibOS::NetworkLibOS(libos) => libos.poll(),
            SingleThreadLibOS::MemoryLibOS(libos) => libos.poll(),
        }
    }
}
