...
impl NetworkRuntime for SharedDPDKRuntime {
    fn transmit(&mut self, mut buf: Box<dyn PacketBuf>) {
        ...
        let mut mbuf = match buf.take_body() {
            Some(mut body) => {
                body.prepend(headers_size.try_into().unwrap()).unwrap();

                body
            }
            None => {
                let frame_size: usize = std::cmp::max(headers_size, MIN_PAYLOAD_SIZE);
                let mut mbuf: DemiBuffer = match self.mm.alloc_mbuf(frame_size) {
                    Ok(mbuf) => mbuf,
        ...
        let mut mbuf_ptr: *mut rte_mbuf = mbuf.into_mbuf().expect("mbuf cannot be empty");
